...
### 0827 (UI 템플릿 적용)
 ...
 - 템플릿 적용 과정은 전체 AI 사용하여 적용

### 0827 (배포 및 호스팅)

 - Vercel을 이용하여 프로젝트 배포


## 수업 및 실습 이후 느낀점

수업 이전에는 웹페이지 제작 자체를 경험해본 적이 없어 단순히 HTML을 이용해 백엔드에서 전달한 API를 보여주는 것으로만 인지하고 있었습니다.

이러한 생각은 수업 과정에서 단계적으로 Vue를 배워면서 완전히 바뀌게 되었습니다. Vue를 배우면서 프런트엔드가 단순히 HTML을 통해 페이지를 사용자에게 제공하는 것이 아닌 브라우저를 통해 백엔드의 부담을 줄이고 사용자에게 원활한 서비스를 제공하도록 하는 부문이라는 것을 알게 되었습니다.

또한 Vue를 이용하여 이전 HTML CSS 과목에서 배웟던 요소들을 Java Script와 함께 하나의 파일 내에서 작성하여 프로젝트의 유연성을 높이는 동시에 Store, Componet 등을 통해 공통적으로 들어가는 인자들을 제어하고 수정하는 방식에 대해서도 보다 자세히 알게 되었습니다.

수업 과정에서 가장 인상깊었던 점은 define, emit, Store 등으로 마치 클래스를 생성하듯이 별도로 분리한뒤, 이를 임포트해와 사용하는 점이었습니다다. 프런트엔드 과정에서 공통 작업을 진행하는 과정에서 어떤식으로 프로젝트 구조와 파일 구조를 구성해야 되는지 아주 작게나마 느낄 수 있어 가장 인상깊게 보면서 실습을 진행하고 깨닳음을 얻었던 것 같습니다.
